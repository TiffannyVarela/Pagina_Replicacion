    ...
    while ($attempt < $maxRetries) {
        try {
            ...
            if (!$conn->connect_error) {
                ...
                if ($attempt > 0) {
                    logError("Conexión MySQL exitosa después de $attempt reintentos", 'INFO');
                }
                return $conn;
            }
            ...
            logError("Intento $attempt: Excepción MySQL: $lastError", 'WARNING');
        }
...
Construye la cadena TNS para conexión Oracle
...
function getOracleConnection($retry = true) {
    ...
    if (!extension_loaded('oci8')) {
        logError('OCI8 extensión no instalada', 'CRITICAL');
        return null;
    }
    ...
    while ($attempt < $maxRetries) {
        try {
            ...
            if ($conn) {
                if ($attempt > 0) {
                    logError("Conexión Oracle exitosa después de $attempt reintentos", 'INFO');
                }
                return $conn;
            }
            //Capturar error de conexion
            ...
            //Capturar excepcion
            $lastError = $e->getMessage();
            logError("Intento $attempt: Excepción Oracle: $lastError", 'WARNING');
        }
        //Incrementar intentos y esperar antes de reintentar
        $attempt++;
        if ($attempt < $maxRetries) {
...
// CONEXIÓN INICIAL MYSQL
$conn_mysql = getMySQLConnection(true);

// CONEXIÓN INICIAL ORACLE
...
function isMySQLConnected() {
    ...
    try {
        ...
        $ping = @$conn->ping();
        if (!$ping) {
            logError("MySQL ping falló", 'WARNING');
        }
        $conn->close();
        return $ping;
    } catch (Exception $e) {
        logError("Excepción en isMySQLConnected: " . $e->getMessage(), 'ERROR');
        return false;
    }
}
...
function executeMySQLQuery($sql, $params = []) {
    ...
    if (!$conn) {
        logError("No hay conexión MySQL para ejecutar query", 'ERROR', ['sql' => $sql]);
